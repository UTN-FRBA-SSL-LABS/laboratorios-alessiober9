...
> R:no coincide con el calculado
...
> R:se encarga de inicializar una estructura de carrito recién creada para que quede vacía y lista para usar
...
> R:este test falla, esperado 700, obtenido 350
...
> R:esta en la línea 24 y dice total += c->items[i].precio;
...
> R:el incorrecto es el >= y el correcto >
...
> R:el test unitario prueba una función  del código para verificar que ande bien por su cuenta en cambio el de  integración prueba cómo interactúan varias funciones o componentes entre sí para ver que todo encaje al combinarse.el test unitario detectó primero el bug de carrito_total (al probar la función suelta con un producto de cantidad 2 en test_total_con_cantidad).
...
> R:en C no existe una verificación automática de límites en memoria, si te pasás del tamaño de un arreglo, el lenguaje igual intenta escribir en esa dirección de RAM, lo que provoca un Buffer Overflow.
...
> R:es mucho mejor porque los ejecutás en un segundo y prueban el código de forma neutra, sin caer en la trampa de leerlo y asumir que está bien solo porque la lógica te suena coherente. además, te quedan armados para verificar que no rompas nada cada vez que hagas un cambio a futuro.
...
> R:El primer test no detectó el bug porque cuando la cantidad es 1, multiplicar el precio por la cantidad o tomar solo el precio da el mismo resultado numérico.
